...
function buildMap(init, age, homePlanet, pastFutureAge) {
  ...
  if (!init) {
    ...
    let date = new Date();
    let explainString = future ? `From now, ${date.getDay()}, ${date.getMonth()} ${date.getDate()} until you are age ${pastFutureAge} in ${homePlanet} years:` : `Since you were age ${pastFutureAge} in ${homePlanet} years:`;
